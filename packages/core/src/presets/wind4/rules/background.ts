...
function bgGradientColorResolver() {
  return function* ([, position, body]: Array<string>, { theme }: RuleContext<Theme>) {
    const css: CSSObject = {};
...
function bgGradientPositionResolver() {
  return function* ([, mode, body]: Array<string>) {
    yield {
...
export const backgroundStyles: Array<Rule<Theme>> = [
  // gradients
