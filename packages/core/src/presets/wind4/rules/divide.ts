...
export const divides: Array<Rule<Theme>> = [
// color & opacity
  ...
  [/^divide-?([xy])-reverse$/, function* ([, d]: Array<string>, { symbols }: RuleContext<Theme>) {
    yield {
      [symbols.selector]: notLastChildSelector,
  ...
  // styles
  [new RegExp(`^divide-(${borderStyles.join('|')})$`), function* ([, style]: Array<string>, { symbols }: RuleContext<Theme>) {
    yield {
      [symbols.selector]: notLastChildSelector,
      'border-style': style,
    };
...
function* handlerDivide([, d, s]: Array<string>, { symbols }: RuleContext<Theme>) {
  ...
  if (v != null) {
    ...
    const directionMap: Record<string, Array<string>> = {
      x: ['-left', '-right'],
      y: ['-top', '-bottom'],
    };
    const results = directionMap[d].map((item): Array<[string, string]> => {
