...
export const sizes: Array<Rule<Theme>> = [
...
export const aspectRatio: Array<Rule<Theme>> = [
  [/^(?:size-)?aspect-(?:ratio-)?(.+)$/, ([, d]: Array<string>) => ({ 'aspect-ratio': getAspectRatio(d) }), { autocomplete: ['aspect-(square|video|ratio)', 'aspect-ratio-(square|video)'] }],
];
