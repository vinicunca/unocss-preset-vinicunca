...
export function directionSize(propertyPrefix: string): DynamicMatcher<Theme> {
  ...
  return (([_, direction, size]: Array<string>): CSSEntries | undefined => {
...
export function colorableShadows(shadows: string | Array<string>, colorVar: string) {
  const colored = [];
...
export function colorCSSGenerator(
...
): [CSSObject, ...Array<CSSValueInput>] | undefined {
  if (!data) {
    return;
  }
  ...
  if (color) {
    const result: [CSSObject, ...Array<CSSValueInput>] = [css];
...
export function colorResolver(property: string, varName: string) {
  return ([, body]: Array<string>, ctx: RuleContext<Theme>): Array<CSSValueInput | string> | undefined => {
    const data = parseColor(body, ctx.theme);
    if (!data) {
      return;
    }

    return colorCSSGenerator(data, property, varName, ctx) as Array<CSSValueInput | string>;
  };
}
...
export function parseThemeColor(theme: Theme, keys: Array<string>) {
...
export function getThemeByKey(theme: Theme, themeKey: keyof Theme, keys: Array<string>) {
...
const resolvedBreakpoints = new WeakMap<any, Array<{ point: string; size: string }>>();
...
export function makeGlobalStaticRules(prefix: string, property?: string): Array<StaticRule> {
...
export function transformXYZ(d: string, v: string, name: string): Array<[string, string]> {
  const values: Array<string> = v.split(splitComma);
