...
import { generator, getUno } from '../utils';
...
describe.concurrent('base', () => {
  describe('animate class names', () => {
    it('"animate-in" should generate enter keyframe and css variables', async ({ expect }) => {
      const { css } = await getUno.generate('animate-in');
    ...
    it('"animate-out" should generate exit keyframe and css variables', async ({ expect }) => {
      const { css } = await getUno.generate('animate-out');
  ...
  describe('options', () => {
    ...
    describe('unit', () => {
      it('should respect the given value', async ({ expect }) => {
        const DURATION = 500;
        const UNIT: NonNullable<AnimationOptions['unit']> = 's';

        const uno = generator({
          presetOptions: {
            animationOptions: { unit: UNIT, duration: DURATION },
          },
        });
        const { css } = await uno.generate('animate-in');
    ...
    describe('fill-mode', () => {
      it('should generate "animation-fill-mode" based on given value', async ({ expect }) => {
        ...
        const uno = generator({
          presetOptions: {
            animationOptions: { fillMode: FILL_MODE },
          },
        });
        const { css } = await uno.generate('animate-in');
    ...
    describe('iteration-count', () => {
      it('should generate "animation-iteration-count" based on given value', async ({ expect }) => {
        ...
        const uno = generator({
          presetOptions: {
            animationOptions: { iterationCount: ITERATION_COUNT },
          },
        });
        const { css } = await uno.generate('animate-in');
    ...
    describe('play-state', () => {
      it('should generate "animation-play-state" based on given value', async ({ expect }) => {
        ...
        const uno = generator({
          presetOptions: {
            animationOptions: { playState: PLAY_STATE },
          },
        });
        const { css } = await uno.generate('animate-in');
    ...
    describe('timing-function', () => {
      it('should generate "animation-timing-function" based on given value', async ({ expect }) => {
        ...
        const uno = generator({
          presetOptions: {
            animationOptions: { timingFunction: TIMING_FUNCTION },
          },
        });
        const { css } = await uno.generate('animate-in');
